"""Charlotte parks scraper.

Source: Charlotte Open Data Portal — Parks (ArcGIS Feature Service)
https://data.charlottenc.gov/maps/735a6bce6306442face38657b50fc7b7

Queries the Mecklenburg County Park and Recreation centroids ArcGIS endpoint,
filtered to CITY = 'CHARLOTTE'.  Returns parks with names, addresses, phone
numbers, park types, and lat/lon coordinates.
"""

from __future__ import annotations

import logging
import re

import requests

logger = logging.getLogger(__name__)

_QUERY_URL = (
    "https://gis.charlottenc.gov/arcgis/rest/services"
    "/HNS/HousingLocationalToolLayers/MapServer/10/query"
)
_HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; NCParksBot/1.0; +https://github.com/nc-parks)"
}

_TYPE_AMENITIES: dict[str, dict[str, bool]] = {
    "community": {},
    "district": {},
    "nature preserve": {"walking_trails": True},
    "neighborhood": {},
    "regional": {},
    "town park": {},
    "greenway": {"walking_trails": True},
}


def _format_phone(raw: str | None) -> str | None:
    if not raw:
        return None
    digits = re.sub(r"\D", "", raw)
    if len(digits) == 10:
        return f"({digits[:3]}) {digits[3:6]}-{digits[6:]}"
    return raw


def fetch() -> list[dict]:
    params = {
        "where": "CITY = 'CHARLOTTE' AND PRKSTATUS = 'Developed'",
        "outFields": "PRKNAME,PRKADDR,CITY,ZIP,PRKTYPE,PRKSTATUS,PRKPHONE,PRKADAACC",
        "outSR": 4326,
        "f": "json",
        "resultRecordCount": 2000,
    }

    resp = requests.get(_QUERY_URL, params=params, headers=_HEADERS, timeout=30)
    resp.raise_for_status()
    data = resp.json()

    parks: list[dict] = []
    for feature in data.get("features", []):
        attrs = feature.get("attributes", {})
        geom = feature.get("geometry", {})

        name = (attrs.get("PRKNAME") or "").strip()
        if not name:
            continue

        address_parts = [attrs.get("PRKADDR", "")]
        city = (attrs.get("CITY") or "Charlotte").strip().title()
        zipcode = attrs.get("ZIP") or ""
        if zipcode:
            address_parts.append(f"{city}, NC {zipcode}")
        else:
            address_parts.append(f"{city}, NC")
        address = ", ".join(p for p in address_parts if p)

        park_type = (attrs.get("PRKTYPE") or "").strip().lower()
        amenities = dict(_TYPE_AMENITIES.get(park_type, {}))
        if attrs.get("PRKADAACC"):
            amenities["ada_accessible"] = True

        lat = geom.get("y")
        lon = geom.get("x")

        source_id = re.sub(r"[^a-z0-9]+", "_", name.lower()).strip("_")
        parks.append({
            "source": "charlotte",
            "source_id": f"charlotte_{source_id}",
            "name": name,
            "latitude": lat,
            "longitude": lon,
            "address": address,
            "city": city,
            "county": "Mecklenburg County",
            "phone": _format_phone(attrs.get("PRKPHONE")),
            "url": "https://parkandrec.mecknc.gov/Places-to-Visit/Parks",
            "amenities": amenities,
            "extras": {"park_type": park_type},
        })

    logger.info("charlotte: fetched %d parks", len(parks))
    return parks


if __name__ == "__main__":
    logging.basicConfig(level=logging.DEBUG, format="%(levelname)s %(message)s")
    parks = fetch()
    print(f"\n{'='*60}")
    print(f"Fetched {len(parks)} Charlotte parks")
    for p in parks:
        coords = f"({p['latitude']:.4f}, {p['longitude']:.4f})" if p.get("latitude") else "N/A"
        print(f"  {p['name']:40s} | {p.get('address',''):50s} | {coords}")